...
<xml xmlns="https://developers.google.com/blockly/xml">
  <block type="he_choose_move" x="40" y="40">
    <comment pinned="false" h="110" w="260">
Return a 7-entry array:
[family, damage, reach, time, status, element, id]

You can use any logic above the return.
    </comment>
    <statement name="DO">
...
function _heEnforceReservedVarsOnEvent(workspace: Blockly.WorkspaceSvg, e: any): void {
  ...
  // NOTE: Some Blockly var events may flush after XML load returns (microtasks),
  // so do NOT rely solely on _heLoadingXml/_heRepairing for correctness.
  ...
  // Helper: if our stored canonical ID is stale (e.g., after Reset/load), treat as missing.
  function _canonicalIdFor(name: HeOutputVarName): string | null {
    const id = _heReservedVarIds[name] || null;
    if (!id) return null;
    const v = vm.getVariableById?.(id);
    return v ? id : null;
  }

  // 1) Block creating any variable with a reserved name (except the canonical var for that name).
  if (t === "var_create") {
    if (_heIsReservedOutputName(varName)) {
      // If we don't yet have a canonical id (or it was stale), adopt the first created one.
      const canonicalId = _canonicalIdFor(varName);
      if (!canonicalId) {
        if (varId) _heReservedVarIds[varName] = varId;
        return;
      }

      // Allow creation of the canonical variable itself.
      if (varId && varId === canonicalId) return;

      // Otherwise: delete the just-created illegal duplicate and show the same UI as the create prompt.
  ...
  if (t === "var_rename") {
    if (_heIsReservedOutputName(newName)) {
      const canonicalId = _canonicalIdFor(newName);
      // If canonical is missing (e.g., during/after load), adopt the target of the rename as canonical.
      if (!canonicalId) {
        if (varId) _heReservedVarIds[newName] = varId;
        // Also force the name to be exactly canonical.
        try {
          if (varId) vm.renameVariableById(varId, newName);
        } catch {}
        return;
      }

      // If renaming the canonical variable to itself, allow.
      if (varId && varId === canonicalId) return;

      // Otherwise, revert the rename and warn.
      try { if (Events?.disable) Events.disable(); } catch {}
      try {
        if (varId && oldName != null) vm.renameVariableById(varId, oldName);
        _heWarnNameExists(2200);
      } finally {
        try { if (Events?.enable) Events.enable(); } catch {}
      }
    ...
    // 3) Block renaming a reserved canonical variable away from its canonical name.
    if (_heIsReservedOutputName(oldName) && oldName) {
      const canonicalId = _canonicalIdFor(oldName);
      if (canonicalId && varId && varId === canonicalId && newName && newName !== oldName) {
        try { if (Events?.disable) Events.disable(); } catch {}
        try {
...
function _heEnsureBlocksRegistered(): void {
  ...
  if (B && B.he_return_move && B.he_choose_move) return;

  // --- Main entry block (hat) ---
  (Blockly as any).Blocks["he_choose_move"] = {
    init: function () {
      this.setColour("#59C059");
      this.appendDummyInput().appendField("Choose My Move");
...
function _heLockMainEntryBlock(main: Blockly.Block): void {
  ...
  try { main.setMovable(false); } catch {}
}

function _heEnsureMainEntryBlock(workspace: Blockly.WorkspaceSvg): Blockly.Block {
  ...
  const mains = all.filter(b => b.type === "he_choose_move");
  let main = mains[0] || null;

  if (!main) {
    const xml = `
      <xml xmlns="https://developers.google.com/blockly/xml">
        <block type="he_choose_move" x="40" y="40"></block>
      </xml>
    `;
...
function _heEnsureReturnShadows(workspace: Blockly.WorkspaceSvg, ret: Blockly.Block): void {
  for (const row of HE_RETURN_INPUTS) {
    ...
    try {
      const field: any = (vget as any).getField?.("VAR");
      if (field && typeof field.setValue === "function") field.setValue(row.varName);
      if (field && typeof field.setVariable === "function") field.setVariable(row.varName);
    } catch {}
...
function _heEnsureTemplate(workspace: Blockly.WorkspaceSvg, reason: string): void {
  ...
  try {
    ...
    const main = _heEnsureMainEntryBlock(workspace);
...
function _loadFromXmlText(xmlText: string): void {
  ...
  // IMPORTANT: Reset/load rebuilds variables with new IDs.
  // Clear canonical ID map so we don't treat freshly-loaded reserved vars as duplicates.
  _heReservedVarIds = {};
